...
def delete(request, aPath, id):
    if not request.user.groups.filter(name="Delete").exists():
        messages.error(request, "You do not have permission to delete items. Contact Admin")
        return redirect(request.META.get('HTTP_REFERER', '/'))
...
def history_view(request, aPath, id):
    aModelClass = None
    ...
    item = aModelClass.objects.filter(id=id).first()
    if not item:
        return HttpResponse("ERR: Object not found")

    if not hasattr(item, "history"):
        return HttpResponse("ERR: HistoricalRecords not enabled on this model")

    history_qs = item.history.all().order_by('-history_date')

    # ✅ PRE-COMPUTE DIFFS HERE (IMPORTANT FIX)
    history_with_diff = []
    ...
    for h in history_qs:
        ...
        if h.prev_record:
            diff = h.diff_against(h.prev_record).changes
    ...
    return render(request, "dyn_dt/history.html", {
        "item": item,
